...
질량, 프로펠러 직경, disk loading과 cant angle을 함께 변화시키는 후속
실험에서 다뤄야 한다.
...
본 단계의 목적은 QuadSim을 실제 연구용 다요인 실험에 사용하기 전에, 각
로터의 추력축을 기체 수직축에서 기울일 수 있도록 동역학과 제어 할당을
확장하고 그 구현 방향이 옳은지 최소한으로 검증하는 것이다.
...
독립변수는 최소한 다음 네 축으로 구성한다.

1. **Cant angle:** 0도를 기준으로 승인된 각도 수준을 대칭 적용
2. **추가 질량:** 실제 장착 가능한 payload 수준과 무게중심 변화를 함께 기록
3. **프로펠러 직경:** 동일 모터/전원 조건에서 허용 가능한 직경 수준
4. **Disk loading:** 질량과 총 rotor disk area로 계산하고 각 실행에 저장

각 조합에서는 호버 RPM, 자세 오차, settling time, 제어 allocation residual,
...
따라서 Cant-Angle MVP는 다음 다요인 시뮬레이션을 시작할 수 있는
baseline-qualified 상태다. 이후 연구에서는 실제 기체 파라미터를 보정하고,
질량, 프로펠러 직경, disk loading과 cant angle을 체계적으로 변화시켜
성능과 한계를 정량화해야 한다.
